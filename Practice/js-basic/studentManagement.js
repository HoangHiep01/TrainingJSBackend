function addStudent(id, name, age) {
	...
	if(students.some((ele) => ele.id == id)){
		console.log("Func addStudent: Id already exists.");
		return -1;
	}
...
function removeStudent(id) {
	...
	if(index == -1){
		console.log("Func removeStudent: Id not found.");
		return -1;
	}

	students.splice(index,1);
	console.log("Func removeStudent: Delete succeseful.");
	return students.length;
...
function addGrade(id, grade) {
	...
	if(index == -1){
		console.log("Func addGrade: Id not found.");
		return -1;
	}
	...
	switch(typeof(grade)){
		...
		case 'string':
			...
			break
		default:
			console.log("Func addGrade: Cannot determine value type.");
			return -1;
	}
...
function getAverageGrade(id) {
	...
	if(index == -1){
		console.log("Func getAverageGrade: Id not found.");
		return -1;
	}
	let result = 0;
	if(students[index].grade.length == 0){
		
		console.log("Func getAverageGrade: Array's length is 0.");
		result = 0;
	}else{
